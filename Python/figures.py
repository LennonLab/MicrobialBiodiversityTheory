...
def figSuppp(figname = 'Fig3', data_dir=mydir, radius=2):
    fig = plt.figure()
    plot_dim = 2
    count = 0
...
def fig3(figname = 'Fig3', data_dir=mydir, radius=2):
    ...
    r2text = r"${}^{{2}}_{{m}} = {:.{p}f} $".format('r',r2_all , p=2)
    plt.text(0.18, 0.93, r2text,  fontsize=14,
        horizontalalignment='center',
        verticalalignment='center',transform = ax.transAxes)
    plt.tick_params(axis='both', which='major', labelsize=7)
    plt.subplots_adjust(wspace=0.5, hspace=0.3)
    ...
    fig.text(0.2, 0.5, r'$Observed\,log_{10}(N_{max})$', ha='center', va='center', rotation='vertical',\
        fontsize = 16)
    fig_name = str(mydir + 'figures/' + figname + '.png')
    plt.savefig(fig_name, dpi=600)#, bbox_inches = 'tight')#, pad_inches=0)
    plt.close()
